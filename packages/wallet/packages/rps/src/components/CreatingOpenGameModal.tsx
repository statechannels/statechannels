...
export default class CreatingOpenGameModal extends React.PureComponent<Props, State> {
  ...
  handleBuyInChange(e) {
    ...
    if (e.target.value === '') {
      ...
      errorMessage = 'Please enter a number for the round buy in';
    } else if (buyIn < MIN_BUYIN || buyIn > MAX_BUYIN) {
      errorMessage = `Invalid round buy in amount ${
        this.state.buyIn
        ...
        <ModalBody>
          <form className="cog-form" onSubmit={e => this.createOpenGameHandler(e)}>
            <div className="form-group">
              ...
              <div className="mt-2">
                Game Buy In: {this.calculateGameBuyIn()} {this.calculateGameBuyIn() ? 'ETH' : ''}
              </div>
              <small className="form-text text-muted">
