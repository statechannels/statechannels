...
export default function MetamaskErrorPage(props: MetamaskErrorProps) {
  ...
  if (props.error.errorType === 'WrongNetwork' && props.error.networkName) {
    message = (
      <span>{`The wrong network is selected in metamask. Please select the ${
        props.error.networkName
      } network in metamask.`}</span>
    );
  }
  ...
  return (
    <ApplicationLayout>
      <div className="waiting-room-container">
        <h1 className="w-100 text-center waiting-room-title">Metamask Error</h1>
        <div className="w-100">
