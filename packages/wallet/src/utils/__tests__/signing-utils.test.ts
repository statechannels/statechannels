import {signCommitment, signData, validSignature, validCommitmentSignature, signVerificationData} from "../../domain";
import {appCommitment, bsPrivateKey, asPrivateKey, asAddress} from "../../domain/commitments/__tests__";
...
describe("signing and validating commitments", () => {
  ...
  it("should return true when a signature is valid", () => {
    ...
    expect(validCommitmentSignature(appCommitment({turnNum: 19}).commitment, commitmentSignature)).toBe(true);
  });
