...
export const hasConsensusBeenReached = (lastCommitment: Commitment, penultimateCommitment: Commitment): boolean => {
...
export const composePostFundCommitment = (lastCommitment: Commitment, ourIndex: TwoPartyPlayerIndex): Commitment => {
  const {channel, turnNum: previousTurnNum, allocation, destination, appAttributes} = lastCommitment;
...
export const composeConcludeCommitment = (channelState: ChannelState) => {
  const commitmentCount = getLastCommitment(channelState).commitmentType === CommitmentType.Conclude ? 1 : 0;
