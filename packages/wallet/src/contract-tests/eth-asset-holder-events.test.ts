...
import {depositFunds, createWatcherState, concludeGame, fiveFive} from "./test-utils";
...
describe("ETHAssetHolder listener", () => {
  ...
  it("should handle a Deposited event", async () => {
    ...
    await depositFunds(provider, channelId, depositAmount.toHexString());
  ...
  it("should handle an AssetTransferred event", async () => {
    ...
    await depositFunds(provider, channelId, depositAmount.mul(2).toHexString());
