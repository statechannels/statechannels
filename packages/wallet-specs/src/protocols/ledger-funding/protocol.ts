...
const assignLedgerChannelId = assign((ctx: Init, event: DoneInvokeEvent<{channelId: string}>) => ({
  ...ctx,
...
export type Services = {
  findLedgerChannelId(ctx: Init): Promise<{type: 'FOUND'; channelId: string} | {type: 'NOT_FOUND'}>;
...
export const guards = {
  channelFound: (_, {data}: DoneInvokeEvent<{type: 'FOUND' | 'NOT_FOUND'}>) => data.type === 'FOUND'
};
