...
class FormInput extends React.Component<FormInputProps, ExtendedFormInputProps> {
  ...
  render() {
    const { className, name, label, max, min, step, type, units, disabled, required, autofocus } = this
    ...
    return (
      <label className={className}>
        ...
        <div className={disabled ? "input-container disabled" : "input-container"}>
          ...
          />
          {Array.isArray(units) && units.length === 1 ? <div className="unit">{units[0].shortName}</div> : null}
        </div>
        {error ? (
