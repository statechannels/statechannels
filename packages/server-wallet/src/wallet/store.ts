...
export class Store {
  ...
  async addSignedState(
    ...
    await timer('validating signatures', async () => recoverParticipantSigners(signedState));
...
function recoverParticipantSigners(signedState: SignedStateWithHash): string[] {
  const {participants} = signedState;

  const participantSigners = signedState.signatures.map(sig => {
    const signerAddress = fastRecoverAddress(signedState, sig.signature, signedState.stateHash);

    // check that each signature was generated by a channel participant
    const validSignature = participants.find(p => p.signingAddress === signerAddress);
    if (!validSignature) {
      throw new StoreError(StoreError.reasons.invalidSignature, {signedState, signature: sig});
    }
    return signerAddress;
  });
  return participantSigners;
}
