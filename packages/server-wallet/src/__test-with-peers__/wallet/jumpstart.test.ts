...
describe('jumpstartObjectives', () => {
  ...
  it('can jumpstart multiple times', async () => {
    ...
    expect(createResponse).toBeObjectiveDoneType('Success');
    expect(jumpstartResponse1).toBeObjectiveDoneType('Success');
    expect(jumpstartResponse2).toBeObjectiveDoneType('Success');
  });
});
