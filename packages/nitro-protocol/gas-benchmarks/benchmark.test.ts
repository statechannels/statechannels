...
describe('Consumes the expected gas for happy-path exits', () => {
  it(`when exiting a directly funded (with ETH) channel`, async () => {
    // begin setup
    await (
      await nitroAdjudicator.deposit(constants.AddressZero, X.channelId, 0, 10, {value: 10})
    ).wait();
    // end setup
    await expect(await X.concludeAndTransferAllAssetsTx(nitroAdjudicator.address)).toConsumeGas(
...
describe('Consumes the expected gas for sad-path exits', () => {
  ...
  it(`when exiting a ledger funded (with ETH) channel`, async () => {
    // begin setup
    await (
      await nitroAdjudicator.deposit(constants.AddressZero, LforX.channelId, 0, 10, {value: 10})
    ).wait();
    // end setup
    // initially                   ⬛ ->  L  ->  X  -> 👩
  ...
  it(`when exiting a virtual funded (with ETH) channel`, async () => {
    // begin setup
    await (
      await nitroAdjudicator.deposit(constants.AddressZero, LforG.channelId, 0, 10, {value: 10})
    ).wait();
    // end setup
    // initially                   ⬛ ->  L  ->  G  ->  J  ->  X  -> 👩
    // challenge L
