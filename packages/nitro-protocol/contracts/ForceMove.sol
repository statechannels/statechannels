...
contract ForceMove {
  ...
  function _stateSupportedBy(
    ...
    require(
      _validSignatures(largestTurnNum, fixedPart.participants, stateHashes, sigs, whoSignedWhat),
      'Invalid signatures'
    );
      ...
      if (i + 1 != variableParts.length) {
        ...
        require(
          _validTransition(
            ...
            [i > variableParts.length - isFinalCount, i + 1 > variableParts.length - isFinalCount], // [a.isFinal, b.isFinal]
  ...
  function _validTransition(
    ...
    if (isFinalAB[1]) {
      ...
      if (turnNumB <= 2 * nParticipants) {
        ...
        require(ForceMoveApp(appDefinition).validTransition(ab[0], ab[1], turnNumB, nParticipants));
        // reason string not necessary (called function will provide reason for reverting)
      }
    }
    return true;
