...
test('concludes on their turn', async () => {
  ...
  await waitForExpect(async () => {
    ...
    expect((await playerA.store.getEntry(channelId)).supportedState.isFinal).toBe(true);
    expect((await playerB.store.getEntry(channelId)).supportedState.isFinal).toBe(true);
  }, 3000);
});
